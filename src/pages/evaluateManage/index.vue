  ...
  <div class="container-box">
    ...
    <div class="scroller-container" :style="{height: WinHeight + 'px'}">
      <scroll-view
        ...
        <div class="order-item" v-for="(item, index) in orderList.data" :key="item.orderid" @click="viewDetail(item)">
          <div class="top-box">
...
<script>
  ...
  export default {
    ...
    methods: {
      viewDetail (item) {
        let url = '../entityOrderDetail/main?orderid=' + item.orderid + '&entryType=' + this.orderStatus
        if (item.order.take === 'V') {
          url = '../orderDetail/main?orderid=' + item.orderid + '&entryType=completed'
        }
        wx.navigateTo({ url })
      },
      tabSwitch (num) {
